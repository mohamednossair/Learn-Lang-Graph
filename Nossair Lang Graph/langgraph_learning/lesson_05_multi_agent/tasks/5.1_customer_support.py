...
def supervisor_node(state: SupportState) -> dict:
    # TODO: call LLM with supervisor prompt + messages
    # Parse response to get routing decision
    # Return {"next": "billing" | "technical" | "returns" | "general" | "FINISH"}
    pass
...
def billing_agent(state: SupportState) -> dict:
    prompt = SystemMessage(content="You are a billing specialist. Resolve payment, invoice, and subscription issues professionally.")
...
def technical_agent(state: SupportState) -> dict:
    prompt = SystemMessage(content="You are a technical support specialist. Resolve bugs, errors, and integration issues with clear steps.")
    response = llm.invoke([prompt] + state["messages"])
...
def returns_agent(state: SupportState) -> dict:
    prompt = SystemMessage(content="You are a returns and refunds specialist. Handle return requests, refunds, and exchanges empathetically.")
    response = llm.invoke([prompt] + state["messages"])
...
def general_agent(state: SupportState) -> dict:
    prompt = SystemMessage(content="You are a general customer support agent. Be helpful, friendly, and resolve queries efficiently.")
    response = llm.invoke([prompt] + state["messages"])
